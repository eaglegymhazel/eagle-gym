...
export function getReviewValidation(input: {
  ...
  const unavailableCount = input.selectedItems.filter((item) => item.isUnavailable).length;
  ...
  const nonCompetitionCount = input.selectedItems.filter((item) => !item.isCompetitionClass).length;
  if (nonCompetitionCount > 0) {
    errors.push(
      `${nonCompetitionCount} selected ${
        nonCompetitionCount === 1 ? "class is" : "classes are"
      } not competition and cannot be booked here.`
    );
  }

  const ageInvalidCount = input.selectedItems.filter((item) => item.ageInvalid).length;
